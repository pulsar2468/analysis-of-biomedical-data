function [mst,adjacency_matrix,e,outlier,error]=create_mst(xtrain,xtest,tr_value,linker);
    ...
    %adjacency_matrix=distance_vector(xtrain{1}); % for euclidean distance
    ...
    [adjacency_matrix,e,outlier,len_training_set_withLinker]=cosine_similarity_test(xtrain{1},linker{2},e,adjacency_matrix,tr_value);
    ...
    %error=1-accuracy;
    sprintf('%d %d %d %d %d %d %d',size(xtrain{1},1),size(xtest{1},1),len_training_set,sensivity,specivity,accuracy,toc)
