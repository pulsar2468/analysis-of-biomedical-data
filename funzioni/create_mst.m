function [mst,adjacency_matrix,e,outlier,error]=create_mst(Y,targets,perc,threshold_percentage,xtrain,xtest);
    ...
    tic;
    [mst,adjacency_matrix,e,outlier,len_training_set]=distance_vector_test(xtrain{1},xtest{1},e,adjacency_matrix,mst,threshold_percentage);
    sprintf('Final size\nTraining: %d\nMst: %d\nE: %d\nThreshold percentage: %d',len_training_set,size(mst,1),size(e,2),threshold_percentage)
